...
public class ExempleController {
	...
	// On retourne toujours un String qui equivaut à la page vers laquelle on veut
	// rediriger.
	// Les objets request et response ne sont plus nécessaire et sont remplacés par
	// Model model.
	@RequestMapping("/display") // Annotation de mapping la plus général (accepte tous les types si non précisé)
	// @RequestMapping(method=RequestMethod.GET, value="")
	// @GetMapping(value="") // c'est la même chose que ci-dessus mais n'accepte que
	// des requêtes de type GET
	// @PostMapping(value="")
	// @PutMapping
	// @PatchMapping
	...
	public String display(Model model) {
		...
		return "exemple"; // mettre directement le nom exacte de la page, spring s'occupe
	...
	// On peut appliquer un filtre qui prend le paramètre que si il respecte la
	// pattern spécifié, le pattern est une expression régulière (REG EX)
	@GetMapping("/exemple2/{param1}/{param2:[0-9]}") // /exemple2/2/3
	public String exemple2_AvecParametres(@PathVariable("param1") String param1, @PathVariable("param2") String param2,
			Model model) {
	...
	public String exemple4_AvecBean(@ModelAttribute("monBean") ExempleForm monBean, Model model) {
		...
		// Avec spring on peut lier une classe Java à un formulaire côté JSP, on les
		// appelle des Beans
		// Cette classe Java doit avoir l'annotation @Component
		// Le nom des attributs de cette classe doit être le même que les noms des
		// champs côté JSP
